...
export class CertificateService {
  constructor() { }
  extractPublicKey(certPem: string) {
    const cert = forge.pki.certificateFromPem(certPem);
    return cert.publicKey;
  }
  extractCommonName(certPem: string): string {
    const cert = forge.pki.certificateFromPem(certPem);
  ...
  private async getECCPassphraseSecret(): Promise<string> {
  ...
  public async generateECCKeyPair(): Promise<KeyPairSyncResult<string, string>> {
    const { publicKey, privateKey } = crypto.generateKeyPairSync('ec', {
      ...
      privateKeyEncoding: {
        ...
        format: 'pem',
        cipher: 'aes-256-cbc',
        passphrase: await this.getECCPassphraseSecret() || process.env.PASSPHRASE
      },
    });
  ...
  public async signData(publicKey: any, privateKey: any, algorithm: string): Promise<string> {
    const sign = crypto.createSign("SHA256");

    if (algorithm === CertConstants.ECC_ALGORITHM) {
      const passphrase = await this.getECCPassphraseSecret() || process.env.PASSPHRASE;
      try {
